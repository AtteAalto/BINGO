...
ea=parameters.ea;
eb=parameters.eb;
eg=parameters.egamma;
eq=parameters.eq;
er=parameters.er;
ebeta=parameters.ebeta;
...
for k=1:its
    ...
    for i=1:n
        ...
        bets=(1-ebeta^2)^.5*betsold(i,:)+ebeta*randn(1,n+n_in);
        beta=.5+.45*bets;
        p_bets=exp(-abs(beta))./exp(-(beta-.5).^2/2/.45^2);
        %p_bets=max(0,beta.*exp(-2*beta.^2))./exp(-(beta-.5).^2/2/.45^2);
        beta=abs(beta);
        
        %Other hyperparameters
        gamma_tr=gamma(i)+eg*nry(i)*randn;
        ...
        PS=sum(S)*log(link_pr)+log(prod(p_bets));
        ...
        if P_aux_ab*P_aux_gamma*exp((PS-Pold(i)+Jold(i)-J1)/Theur) > rand
            Sold(i,:)=S;
            Pold(i)=PS;
            Jold(i)=J1;
            betsold(i,:)=bets;
        ...
        if isfield(data,'missing')  %Missing measurements
            if size(data.missing{l},1)>.5
                for i=1:n
                    ...
                    coef=(1-ee^2)^.5*nomiss(i,Ser(1,l):Ser(2,l))+(qtr(i)/q(i))^.5*(1-nomiss(i,Ser(1,l):Ser(2,l)));
